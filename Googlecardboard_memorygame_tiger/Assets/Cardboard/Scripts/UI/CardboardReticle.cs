...
public class CardboardReticle : MonoBehaviour, ICardboardGazePointer
{
	                        ...
	                        bool isInteractive)
	{
		...
		m_reticle.fillAmount = m_gazeTimer / gazeDelay;
	}
	...
	public void OnGazeExit (Camera camera, GameObject targetObject)
	{
		...
		m_reticle.fillAmount = m_gazeTimer = 0f;
	...
	//
		innerRadius = 0.0f; //2.0f * Mathf.Tan(min_inner_angle_radians);
		outerRadius = 0.5f; //2.0f * Mathf.Tan(max_inner_angle_radians);
	}
